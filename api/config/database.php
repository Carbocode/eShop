...
class DatabaseService
{
    ...
    public function getConnection()
    {
        ...
        $this->connection->query('CREATE DATABASE IF NOT EXISTS DefaultCube');
        $this->connection->query('USE DefaultCube');
        $this->connection->query('CREATE TABLE IF NOT EXISTS account(
        ...
        $this->connection->query('CREATE TABLE IF NOT EXISTS products(
            ...
            weightMB DOUBLE,
            rating INT)');
        $this->connection->query('CREATE TABLE IF NOT EXISTS ordini(
            username VARCHAR(30),
            id_prod INT,
            rating INT,
            PRIMARY KEY(username, id_prod))');
        $this->connection->query('CREATE TABLE IF NOT EXISTS popularity(
            id_prod INT PRIMARY KEY,
            views INT,
            downloads INT)');
